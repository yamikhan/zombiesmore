...
public class ZombieCreeper extends Creeper {
	...
	public void aiStep() {
		...
		if (flag) {
			...
			if (flag) {
				this.setRemainingFireTicks(160); // 8 seconds * 20 ticks = 160
			}
		}
	...
	public void tick() {
		if (!this.level().isClientSide && this.isAlive() && this.isConverting()) {
			...
			if (this.creeperConversionTime <= 0 && EventHooks.canLivingConvert(this, EntityType.CREEPER, (timer) -> this.creeperConversionTime = timer)) {
	...
	private void finishConversion(ServerLevel serverLevel) {
		...
		if (creeperEntity != null) {
			...
			EventHooks.onLivingConvert(this, creeperEntity);
		}
	}
